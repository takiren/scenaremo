...
defaults:
  ...
  sceneGapMs: 500       # シーンの末尾（＝シーン間と動画末尾）の余白
...
| `defaults.sceneGapMs` | シーンの末尾。シーン間の間（ま）と動画末尾の余韻の両方 | 500ms |

`sceneGapMs` は**次のシーンの頭ではなく前のシーンの尻に付きます**。繋ぎは次のシーンの先頭で行われ、
そのぶん前のシーンの末尾に重なるので、余白を尻に置くと**フェードが無音の中で完結します**。
...
既定の 500ms は、既定のトランジション 400ms より長く（フェードが語尾に被らない）、
かつセリフ間の 300ms より長い（話題の切れ目はセリフの切れ目より深く間を取る）ように選んでいます。
