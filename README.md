...
# color は字幕の色。立ち絵が無い構成では、話者が変わったことは字幕の色でしか分からない
speakers:
  zundamon:
    engine: voicevox
    styleId: 3
...
scenes:
  - image: assets/01-title.png
    transition: fade
    lines:
      # 読み上げる文字列 (text) と、字幕に出す文字列 (caption) は分けられる
      - text: 今日はリモーションの話をするのだ
        caption: 今日は Remotion の話をするのだ
      - speaker: metan
        text: |
          スライドショー形式の
          解説動画を作りますね
...
セリフはそのまま字幕になります。画面の一等地は字幕に渡してあり、立ち絵より優先します。
解説動画で扱うのは視聴者が聞いたことのない言葉であり、**知らない言葉は音だけでは聞き取れない**ためです。
そのぶん、字幕として成立するかどうかは台本の書き方に懸かっています。

**1 行は短く書いてください。** 長すぎる行を CLI が自動で分割することはありません。
行の途中で切るには行内のタイミングが要り、それを持ち込むと台本に書いていない切れ目が生まれます。
表示幅での折り返しは自動で行われるので、収まらない行は画面下部を何行も占領したまま長く出続けます。
`text` に書いた改行はそのまま字幕の改行になるので、切れ目を自分で決めることもできます。

**読み仮名が要る行には `caption` を書いてください。**
聞いたことのない言葉ほど VOICEVOX は正しく読みません。`Kubernetes` は英語のままでは読めず、`冪等` も怪しいので、
`text` には読み仮名を書くことになります。ところがその文字列をそのまま字幕に出すと、
視聴者は「クーベルネティス」を受け取って帰ることになり、**その言葉を検索できません**。
字幕を入れる理由が「知らない言葉を受け取らせること」なら、字幕は綴りのほうで出ていなければ目的を達成しません。
`caption` を省略した行は `text` がそのまま字幕になるので、読み仮名が要らない行では書かなくて構いません。

話者は `speakers[].color` の色で区別します。名前ラベル（`ずんだもん:`）は横幅を食うためです。
ただし**色だけに意味を持たせないでください。** 色覚特性によっては区別できない人がいるので、
誰が喋っているかがその場面で本質的な情報なら、そのことは台本の文面でも示してください。

見た目（帯の濃さ・文字の大きさ・位置）は [`renderer/src/Subtitle.tsx`](renderer/src/Subtitle.tsx) にあり、書き換える前提の層です。
字幕は音声と同じくシーンコンポーネントの外側の共通レイヤーなので、自作のシーンコンポーネントを書いても消えません。
...
| **Phase 2** | VOICEVOX の自動起動 / Docker 対応、縦型(9:16)出力、BGM・効果音、立ち絵、口パク |
...
台本から使用話者を機械的に集計し、**既定で動画の末尾にクレジットシーンを自動挿入します**。
書き忘れたときに黙って抜けるより、要らない人が明示的に切るほうが安全側に倒れるためです。
**表記漏れによる事故を防ぐことは、この CLI の重要な役割**と位置づけています。

```yaml
meta:
  creditsScene: false   # 末尾のクレジットシーンを入れない（既定は true）
```

切っても集計そのものは止まりません。`scenaremo credits` で表記を取り出して概要欄に貼る、
オープニングへ自分で置く、といった使い方はできますが、**切った場合の表記はあなたの責任になります。**

クレジットシーンの尺は表記の件数から CLI が決めます（台本にフレーム数も秒数も書かせないため）。
見た目は `renderer/src/Credits.tsx` にあり、他の見た目と同じくあなたが書き換えられます。
