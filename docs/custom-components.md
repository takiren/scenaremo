...
### 1. 音声と字幕は共通レイヤーに任せる
シーンコンポーネントの責務は **背景や図解・画像のアニメーション表現** です。音声再生 (`SceneAudio.tsx`) と字幕描画 (`Subtitle.tsx`) は共通の [`Slideshow.tsx`](../renderer/src/Slideshow.tsx) 側でシーンコンポーネントの外に置かれているため、個別のシーンコンポーネント内で音声や字幕の描画を行う必要はありません。

逆に言えば、**シーンコンポーネントを書いても字幕は消えません**。字幕はシーンの上に重なるので、画面下部（既定では下から 8% の位置）は字幕に明け渡すつもりで構図を決めてください。字幕の見た目そのものを変えたい場合は、コンポーネントを増やすのではなく `Subtitle.tsx` を書き換えます（すべての動画に効きます）。
