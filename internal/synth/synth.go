...
//
// 処理は Workers の数だけ goroutine を立てて並べる（→ issue #24）。
// 既定の 1 では台本の順に 1 件ずつ進む。
//
// 同じセリフ（同じ話者・同じパラメータ・同じテキスト）が台本に 2 回現れた場合、
// キーは合成の入力だけで決まるので、Audio には両方とも同じパスが載る。
// 順次なら 2 回目は 1 回目が保存したものを読んでキャッシュヒットになるが、
// 並列では両方ともキャッシュを空振りし、同じキーへ同時に保存しに行くことがある。
// 出来上がる wav は同一なのでどちらが残ってもよいが、Store はその同時保存に耐える必要がある
// （→ Store のコメント）。
//
